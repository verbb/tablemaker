...
(function ($) {
    ...
    Craft.TableMaker = Garnish.Base.extend({
        ...
        initRowsTable: function (columns) {
            ...
            this.bindRowsTableTextChanges();
            this.bindRowsTableAlignment();
            this.queueRowsTableAlignment();

            this.rowsTable.sorter.settings.onSortChange = $.proxy(this, 'makeDataBlob');
        },

        bindRowsTableAlignment: function () {
            var s = this;
            var $table = this.rowsTable ? this.rowsTable.$table : null;

            if (!$table || !$table.length) {
                return;
            }

            $table.off('.tablemaker-align');
            $table.on('focusin.tablemaker-align click.tablemaker-align mousedown.tablemaker-align', 'td, textarea, input, select', function () {
                s.queueRowsTableAlignment();
            });
        },

        queueRowsTableAlignment: function () {
            if (this.alignmentTimeout) {
                clearTimeout(this.alignmentTimeout);
            }

            this.alignmentTimeout = setTimeout($.proxy(function () {
                this.applyRowsTableAlignment();
            }, this), 0);
        },

        getColumnAlign: function (column) {
            var align = column && column.align ? column.align.toLowerCase() : '';
        ...
        applyRowsTableAlignment: function ($table) {
            var $targetTable = $table || (this.rowsTable ? this.rowsTable.$table : null);
            ...
            for (var colId in this.columns) {
                aligns.push(this.getColumnAlign(this.columns[colId]));
            }

            var $headerCells = $targetTable.find('thead tr').first().children('th').not('[colspan]');

            $headerCells.each(function (index, el) {
                if (aligns[index]) {
                    $(el).css('text-align', aligns[index]);
                } else {
                    $(el).css('text-align', '');
                }
            });

            $targetTable.find('tbody tr').each(function (_, rowEl) {
                var $cells = $(rowEl).children('td');

                for (var i = 0; i < aligns.length; i++) {
                    var $cell = $cells.eq(i);
                    var $cellChildren = $cell.children();
                    var $preview = $cell.find('.editable-table-preview');
                    var $textControls = $cell.find('textarea, .select select, input[type="text"], input[type="email"], input[type="url"], input[type="number"], input[type="search"], input[type="tel"], input:not([type])');

                    if (aligns[i]) {
                        $cell.css('text-align', aligns[i]);
                        $cellChildren.css('text-align', aligns[i]);
                        $preview.css('text-align', aligns[i]);
                        $textControls.css('text-align', aligns[i]);
                    } else {
                        $cell.css('text-align', '');
                        $cellChildren.css('text-align', '');
                        $preview.css('text-align', '');
                        $textControls.css('text-align', '');
                    }
                }
            });
        },
        ...
        reconstructRowsTable: function () {
            ...
            for (var colId in this.columns) {
                var align = this.getColumnAlign(this.columns[colId]);
                var style = align ? ' style="text-align: ' + align + ';"' : '';
                tableHtml += '<th scope="col" class="header"' + style + '>' + (this.columns[colId].heading ? this.columns[colId].heading : '&nbsp;') + '</th>';
            }
            ...
            this.applyRowsTableAlignment($table);
